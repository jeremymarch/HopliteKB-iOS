...
int combiningAccents[NUM_COMBINING_ACCENTS] = { COMBINING_MACRON, COMBINING_DIAERESIS, COMBINING_CIRCUMFLEX, COMBINING_ROUGH_BREATHING, COMBINING_SMOOTH_BREATHING, COMBINING_ACUTE, COMBINING_GRAVE, COMBINING_IOTA_SUBSCRIPT };
...
void accentCodeToAnalysis(int accentCode, bool *smooth, bool *rough, bool *acute, bool *grave, bool *circumflex, bool *iota_sub, bool *diaeresis)
{
    *smooth = false;
    *rough = false;
    *acute = false;
    ...
    *diaeresis = false;
    ...
    else if (accentCode == DIALYTIKA)
    {
        *diaeresis = true;
    }
    else if (accentCode == DIALYTIKA_AND_OXIA)
    {
        *diaeresis = true;
        *acute = true;
    }
    else if (accentCode == DIALYTIKA_AND_VARIA)
    {
        *diaeresis = true;
        *grave = true;
    }
    else if (accentCode == DIALYTIKA_AND_PERISPOMENON)
    {
        *diaeresis = true;
        *circumflex = true;
    }
...
short getPrecomposedLetter(int letterCode, bool smooth, bool rough, bool acute, bool grave, bool circumflex, bool iota_sub, bool macron, bool diaeresis)
{
    int accent = 0;
    
    if (!smooth && !rough && !acute && !grave && !circumflex && !iota_sub && !macron && !diaeresis)
        accent = NORMAL;
...
void accentSyllable2(UCS2 *ucs2String, int i, int *len, int accent, bool toggleOff)
{
    ...
    else
    {
        ...
        accentCodeToAnalysis(accentCode, &smooth, &rough, &acute, &grave, &circumflex, &iota_sub, &diaeresis);
    }
    ...
    if (macron || onlyUseCombiningDiacritics)// || diaeresis || onlyUseCombiningDiacritics) //if there is a macron we use combining accents, else precomposed
    {
        if (macron)
            newLetterLen++;
        if (smooth)
        ...
        for (int k = 0; k < NUM_COMBINING_ACCENTS; k++)
        {
            ...
            else if (combiningAccents[k] == COMBINING_DIAERESIS && diaeresis)
            {
                ucs2String[i + numAccents] = combiningAccents[k];
                numAccents++;
            }
    ...
    else
    {
        ...
        short ucs2 = getPrecomposedLetter(letterCode, smooth, rough, acute, grave, circumflex, iota_sub, macron, diaeresis);
        if (ucs2 != 0)
        {
            ucs2String[i] = ucs2;
        }
